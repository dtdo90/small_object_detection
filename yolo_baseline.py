...
class InferenceBaseline:
    ...
    def process_frame(self,frames):
        ...
        with torch.no_grad():  # Disable gradient calculation
            results = self.detection_model(frames)
    ...
    def inference_video(self, weights, source, buffer_size):
        ...
        if type(source) is not int: # video input
            ...
            out = cv2.VideoWriter(source[:-4] + "_baseline_batch.mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height))
            ...
            while frame_count < num_frames:
                ...
                # Calculate running FPS (only every 16 frames)
                if frame_count % 16 == 0:
                    avg_time = sum(processing_times[-16:]) / len(processing_times[-16:])
                    current_fps = 1 / avg_time if avg_time > 0 else 0
                    print(f"Frame {frame_count}/{num_frames}: {current_fps:.2f}fps")
